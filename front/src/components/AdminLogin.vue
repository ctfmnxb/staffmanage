    ...
    <div class="login-box-wrapper">
      <div class="login-box animated-login-box">
        ...
        <form @submit.prevent="handleLogin">
          ...
          <button type="button" @click="(e) => { goToUserLogin(); showClickFeedback(e.target); }" class="user-login-btn">前往用户登录</button>
    ...
    async handleLogin() {
      ...
      try {
        ...
        if (response.data.success) {
          ...
          this.$alert(response.data.message || '登录失败，请检查用户名和密码', '登录错误', {
            confirmButtonText: '确定',
            type: 'error'
          });
        }
      } catch (error) {
        this.$alert(error.response?.data?.message || '登录出错，请稍后重试', '登录错误', {
          confirmButtonText: '确定',
          type: 'error'
        });
      }
